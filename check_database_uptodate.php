...
if ($verif==0) {
	...
	$req="select table_name from all_tables where table_name ='DWH_VIRTUAL_PATIENT' ";
	$sel = oci_parse($dbh_etl,$req);
	oci_execute($sel);
	$ligne = oci_fetch_array($sel);
	...
	$req="select index_name from all_indexes where index_name ='DWH_ECRF_USER_RIGHT_I' ";
	$sel = oci_parse($dbh_etl,$req);
	oci_execute($sel);
	$ligne = oci_fetch_array($sel);
