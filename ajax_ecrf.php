	...
	if ($autorisation_ecrf_modifier=='ok') {
  		...
  		foreach ($tableau_item as $item) {
  			...
  			if ($list_sub_item_str!='') {
				...
				$ecrf_item_num=insert_ecrf_item ($ecrf_num ,$item_str,$item_type,'',$item_ext_name,$item_ext_code,$regexp, $item_local_code,$regexp_index,$period,$item_order) ;
				...
				if ($item_str!='' && $item_type!='' ) {
	  				$max_item_order=get_max_ecrf_item_order ($ecrf_num);
	  				$item_order=$max_item_order+1;
					insert_ecrf_item ($ecrf_num ,$item_str,$item_type,'',$item_ext_name,$item_ext_code,$regexp, $item_local_code,$regexp_index,$period,$item_order) ;
		  		}
		  	}
	  	}
  	}
...
if ($_POST['action']=='add_new_item') {
	...
	if ($autorisation_ecrf_modifier=='ok') {
		$max_item_order=get_max_ecrf_item_order($ecrf_num);
		$item_order=$max_item_order+1;
		$ecrf_item_num=insert_ecrf_item ($ecrf_num ,$item_str,$item_type,$item_list,$item_ext_name,$item_ext_code,$regexp,$item_local_code,$regexp_index,$period,$item_order) ;
  	}
  	print "$ecrf_item_num";
}
...
if ($_POST['action']=='afficher_onglet_ecrf_patient') {
	...
	print "<select id=\"id_select_ecrf\" class=\"form chosen-select\">";
	...
	if ($autorisation_ecrf_voir=='ok') {
		...
		foreach ($tableau_list_ecrf_items as $item) {
			...
			insert_result_auto_ecrf($patient_num,$ecrf_num,$ecrf_item_num,$user_num_session,$table_value,$table_appercu,$final_automatic_value);
		 	...
		 	print "<td style=\"vertical-align:top\"><div onmouseup=\"ecrf_justify_my_choice();\">";
