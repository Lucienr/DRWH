...
function parcours_patient ($patient_num) {
	...
	parcours_complet('','',$patient_num,'department',0);
	
	if (is_file("$CHEMIN_GLOBAL/upload/tmp_graphviz_parcours_sejour_service_$patient_num.png")) {
		$parcours_sejour_service='ok';
	}
	if (is_file("$CHEMIN_GLOBAL/upload/tmp_graphviz_parcours_sejour_uf_$patient_num.png")) {
		$parcours_sejour_uf='ok';
	}
	if (is_file("$CHEMIN_GLOBAL/upload/tmp_graphviz_parcours_complet_$patient_num.png")) {
		$parcours_complet='ok';
	}
	print "
	<script language=javascript>
		function afficher_parcours (id) {";
			...
			if ($parcours_complet=='ok') {print "$('#id_lien_complet').css('font-weight','normal');";}
			...
			if ($parcours_complet=='ok') {print "$('#id_div_img_parcours_complet').css('display','none');";}
			print "$('#id_div_img_parcours_'+id).css('display','block');";
	print "}
	</script>";
	if ($parcours_sejour_service=='ok') {print "<span id=\"id_lien_department\" style=\"font-weight:bold;\"><a  href=\"#\" onclick=\"afficher_parcours('department');\">".get_translation('DISPLAY_JOURNEY_STAY_BY_HOSPITAL_DEPARTMENT','Afficher parcours séjour par service')."</a></span> - ";}
	if ($parcours_sejour_uf=='ok') {print "<span id=\"id_lien_unit\" style=\"font-weight:normal;\"><a  href=\"#\"  onclick=\"afficher_parcours('unit');\">".get_translation('DISPLAY_JOURNEY_STAY_BY_HOSPITAL_UNIT','Afficher parcours séjour par UF')."</a></span> -  ";}
	if ($parcours_complet=='ok') {print "<span id=\"id_lien_complet\" style=\"font-weight:normal;\"><a  href=\"#\"  onclick=\"afficher_parcours('complet');\">".get_translation('DISPLAY_ALL_MOVEMENT_INCLUDING_CONSULT','Afficher tous les passage (consultation comprise)')."</a></span> ";}
	print "<br> ";
	if ($parcours_sejour_service=='ok') {print "<div id=\"id_div_img_parcours_department\" style=\"display:block\"><img src=\"upload/tmp_graphviz_parcours_sejour_service_$patient_num.png\"></div> ";}
	if ($parcours_sejour_uf=='ok') {print "<div id=\"id_div_img_parcours_unit\" style=\"display:none\"><img src=\"upload/tmp_graphviz_parcours_sejour_uf_$patient_num.png\"></div> ";}
	if ($parcours_complet=='ok') {print "<div id=\"id_div_img_parcours_complet\" style=\"display:none\"><img src=\"upload/tmp_graphviz_parcours_complet_$patient_num.png\"></div> ";}
...
function affiche_tableau_biologie($patient_num) {
	...
	print "
<table id=\"id_tableau_bilan_biologie\" class=\"tablefin\">";
	print "<thead>";
		print "<tr>";
			print "<th></th>";
