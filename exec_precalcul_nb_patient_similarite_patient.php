...
if ($requete!='') {
	...
	if ($requete_exclure!='') {
		...
		while ($r=oci_fetch_array($sel,OCI_RETURN_NULLS+OCI_ASSOC)) {
			$patient_num=$r['PATIENT_NUM'];
			if ($tab_patient_num_deja[$patient_num]=='ok') {
		        	$ins= oci_parse($dbh,"delete from dwh_process_patient where process_num='$process_num' and patient_num=$patient_num");   
			        oci_execute($ins);
				$tab_patient_num_deja[$patient_num]='';
			}
		}
	}
}

if ($tab_patient_num_deja[$patient_num_principal]=='') {
	$ins= oci_parse($dbh,"insert into dwh_process_patient (process_num,patient_num) values ('$process_num','$patient_num_principal') " );   
	oci_execute($ins);
}
